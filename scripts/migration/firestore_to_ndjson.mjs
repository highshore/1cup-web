// Dump every Firestore collection (running in the emulator) to NDJSON.
//
// Usage:
//   FIRESTORE_EMULATOR_HOST=127.0.0.1:8080 GOOGLE_CLOUD_PROJECT=demo-onecup \
//     node firestore_to_ndjson.mjs
//
// Output: ../data/<collection>.ndjson  (one JSON object per line)
//         plus subcollections: article_meanings, meetup_report_users, speaking_reports
...
if (!process.env.FIRESTORE_EMULATOR_HOST) {
  console.error("Refusing to run: set FIRESTORE_EMULATOR_HOST (point at the emulator).");
  process.exit(1);
}

initializeApp({ projectId: process.env.GOOGLE_CLOUD_PROJECT || "demo-onecup" });
