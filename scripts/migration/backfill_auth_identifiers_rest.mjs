// Backfill public.users.phone / email from Firebase Auth, over PostgREST.
//
...
do {
  ...
  res.users.forEach((u) => fb.set(u.uid, { phone: toLocal(u.phoneNumber), email: u.email || null }));
  pageToken = res.pageToken;
} while (pageToken);

const { data: rows, error } = await sb.from("users").select("uid, phone, email").limit(5000);
if (error) throw error;

const phone = [], email = [], conflict = [];
for (const r of rows) {
...
console.log(`phone fillable: ${phone.length}   email fillable: ${email.length}   conflicts: ${conflict.length}`);
