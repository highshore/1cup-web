...
/**
 * Import English Wiktionary data exported by Wiktextract/Kaikki into Supabase.
 *
 * Expected input: raw Wiktextract JSONL or JSONL.GZ from
 ...
 *
 * Examples:
 *   node scripts/import_wiktionary_dictionary.mjs ./kaikki.org-dictionary-English.jsonl.gz
 *   node scripts/import_wiktionary_dictionary.mjs ./extract.jsonl.gz --terms "discreet,overtly,vouch for"
 *   node scripts/import_wiktionary_dictionary.mjs ./extract.jsonl.gz --terms-file ./terms.txt
 */
...
if (inlineTerms) {
  inlineTerms.split(",").map(normalize).filter(Boolean).forEach((term) => requestedTerms.add(term));
}
const termsFile = argValue("--terms-file");
if (termsFile) {
  fs.readFileSync(termsFile, "utf8")
    .split(/\r?\n/)
    .map(normalize)
    .filter(Boolean)
    .forEach((term) => requestedTerms.add(term));
}
...
const relationWords = (items) => [...new Set(
...
const stableFallbackId = (word, pos, order, gloss) => {
  const digest = createHash("sha1")
    .update(`${normalize(word)}|${pos || "unknown"}|${order}|${gloss}`)
    .digest("hex");
  return `wiktextract:${digest}`;
};
...
const flushBatch = async () => {
  ...
  const normalizedTerms = entries.map((entry) => entry.normalized_term);
  const { data: entryRows, error: entryLookupError } = await supabase
    .from("dictionary_entries")
  ...
  for (const record of records) {
    ...
    const pronunciation = (Array.isArray(record.sounds) ? record.sounds : [])
      .find((sound) => typeof sound?.ipa === "string" && sound.ipa.trim())?.ipa || null;
    ...
    senses.forEach((sense, order) => {
      ...
      const gloss = (Array.isArray(sense?.glosses) ? sense.glosses : [])
      ...
      meanings.push({
        entry_id: entryId,
        source_meaning_id: typeof sense.id === "string" && sense.id.trim()
          ? sense.id.trim()
          : stableFallbackId(term, record.pos, order, gloss),
  ...
  if (meanings.length > 0) {
    const { error: meaningError } = await supabase
      ...
      .upsert(meanings, { onConflict: "source,source_meaning_id" });
...
for await (const line of lines) {
  ...
  if (!normalized || (requestedTerms.size > 0 && !requestedTerms.has(normalized))) {
    skippedRecords += 1;
    continue;
  }
