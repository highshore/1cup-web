-- Erase a member's data in one transaction.
--
-- The Edge Function did this as about twenty separate PostgREST calls. Any one of them
-- failing left the account half-erased, and because the function refuses to run for an
-- account already marked deleted, the half-erased state could not be retried: the auth
-- identity would sit there permanently, able to sign in to a profile that had been
-- emptied. That is worse than the missing atomicity itself.
--
-- Everything that lives in Postgres now happens here, so it either all lands or none of
-- it does. Storage and the auth identities stay outside — they are not Postgres and
-- cannot join the transaction — which is why this runs first and they follow.
...
begin
  ...
  -- Leave meetups that have not happened yet and correct their counts. Past attendance
  -- stays: it is meeting history against a uid that no longer names anyone.
  ...
  -- Social graph and private content.
  ...
  -- The member's own records. The privacy policy promises the study record goes at
  -- withdrawal, and answers and recordings are not something to keep for someone who left.
  ...
  -- Keyed by phone rather than uid, so no uid-based cleanup was ever going to reach it.
  if v_phone is not null and btrim(v_phone) <> '' then
    delete from public.phone_otp where phone = v_phone;
  end if;

  -- Kept for accounting, stripped of anything describing a person.
  ...
  -- Not privacy but correctness: the code stayed redeemable and still credited someone
  -- who no longer exists.
  update public.referral_codes set active = false, referrer = null where referrer = p_uid;

  -- The tombstone. uid survives so payment history and past meetups stay referentially
  -- intact; everything that named a person does not.
  update public.users
     set email = null, display_name = 'Deleted member', photo_url = null, phone = null,
         ...
         -- location is NOT NULL DEFAULT 'anam'. Writing null here is what made every
         -- deletion fail: the profile update threw, the caller saw account_deletion_failed,
         -- and not one account has ever been erased. Back to the default instead.
         location = 'anam',
