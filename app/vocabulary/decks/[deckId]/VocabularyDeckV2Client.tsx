...
const copyByLocale = {
  ko: {
    ...
    addWord: "단어 추가",
    addTitle: "단어 추가",
    addHint: "글로벌 Wiktionary 사전에서 단어를 검색하고 원하는 뜻을 골라 추가하세요.",
  ...
  en: {
    ...
    addHint: "Search the global Wiktionary dictionary and add the exact meaning you want.",
...
export default function VocabularyDeckV2Client({ deckId }:{ deckId:string }) {
  ...
  const [addOpen,setAddOpen]=useState(false); const [editOpen,setEditOpen]=useState(false); const [query,setQuery]=useState(""); const [searching,setSearching]=useState(false); const [results,setResults]=useState<DictionaryEntry[]>([]); const [busyKey,setBusyKey]=useState<string|null>(null);
  ...
  useEffect(()=>{ if(!addOpen||!isOwner){setResults([]);return} const normalized=query.trim().toLowerCase(); if(normalized.length<2){setResults([]);return} const timer=window.setTimeout(async()=>{setSearching(true);try{ const { data:entries,error:entryError }=await supabase.from("dictionary_entries").select("id,term,normalized_term").eq("language_code","en").ilike("normalized_term",`${normalized}%`).order("normalized_term",{ascending:true}).limit(20); if(entryError)throw entryError; const ids=(entries||[]).map((row:any)=>String(row.id)); if(!ids.length){setResults([]);return} const { data:meaningRows,error:meaningError }=await supabase.from("dictionary_meanings").select("id,entry_id,grammar_type,definition_en,definition_ko,pronunciation_ipa,example_en,example_ko,audio_url,image_url,source,meaning_order").in("entry_id",ids).order("meaning_order",{ascending:true}); if(meaningError)throw meaningError; const byEntry:Record<string,Meaning[]>={}; (meaningRows||[]).forEach((row:any)=>{const meaning=mapMeaning(row);if(!meaning)return;(byEntry[meaning.entryId]??=[]).push(meaning)}); Object.values(byEntry).forEach(list=>list.sort((a,b)=>(a.source==="wiktionary"?0:1)-(b.source==="wiktionary"?0:1)||a.meaningOrder-b.meaningOrder)); setResults((entries||[]).map((row:any)=>{const all=byEntry[String(row.id)]||[];const wiki=all.filter(m=>m.source==="wiktionary");return{id:String(row.id),term:String(row.term),meanings:wiki.length?wiki:all}})); }catch(failure){console.error("Dictionary search failed",failure);setResults([])}finally{setSearching(false)}},220); return()=>window.clearTimeout(timer); },[addOpen,isOwner,query]);
  ...
  return <Page><Shell>
    ...
    {addOpen&&<ModalBackdrop onClick={()=>setAddOpen(false)}><Modal onClick={event=>event.stopPropagation()}><ModalHeader><div><ModalTitle>{copy.addTitle}</ModalTitle><Hint>{copy.addHint}</Hint></div><Close type="button" onClick={()=>setAddOpen(false)}><XMarkIcon /></Close></ModalHeader><Search autoFocus value={query} onChange={event=>setQuery(event.target.value)} placeholder={copy.search}/>{query.trim().length<2?<Empty style={{marginTop:12}}>{copy.searchStart}</Empty>:searching?<Empty style={{marginTop:12}}>{copy.loading}</Empty>:results.length?<Results>{results.map(entry=><Result key={entry.id}><Term>{entry.term}</Term>{entry.meanings.length?entry.meanings.map(meaning=>{const key=`${entry.id}:${meaning.id}`;const exists=itemKeys.has(key);return <Sense key={meaning.id}><SenseTop><div><Grammar>{meaning.grammarType}</Grammar><Definition>{meaning.definitionEn||copy.noDefinition}</Definition>{meaning.definitionKo&&<Korean>{meaning.definitionKo}</Korean>}</div><Button type="button" $active={exists} disabled={exists||busyKey===key} onClick={()=>void addMeaning(entry,meaning)}>{exists?<CheckIcon />:<PlusIcon />}{exists?copy.added:copy.add}</Button></SenseTop></Sense> }):<Sense><SenseTop><Definition>{copy.noDefinition}</Definition><Button type="button" onClick={()=>void addMeaning(entry,null)}><PlusIcon />{copy.add}</Button></SenseTop></Sense>}</Result>)}</Results>:<Empty style={{marginTop:12}}>{copy.noSearch}</Empty>}</Modal></ModalBackdrop>}
