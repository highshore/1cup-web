...
function readAudience(idToken: string): string[] | null {
  try {
    ...
    if (Array.isArray(payload.aud)) return payload.aud.filter((value) => typeof value === "string");
    return null;
  } catch {
    return null;
  }
...
export async function GET(request: NextRequest) {
  ...
  // Supabase is contacted only from Vercel here. The browser never navigates to
  // *.supabase.co, avoiding the certificate/trust failure seen on desktop/mobile.
  ...
  // Preserve PR20's existing account reconciliation behavior for migrated members.
