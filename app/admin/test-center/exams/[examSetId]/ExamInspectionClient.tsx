...
export default function ExamInspectionClient({ examSetId }: { examSetId: string }) {
  ...
  return <ExamPage>
    ...
    <Summary>
      <SummaryCard><SummaryTop><div><Eyebrow>{t.examCenter.readinessGate}</Eyebrow><h2 style={{ margin: 0, fontSize: 21, fontWeight: 900, letterSpacing: "-.04em" }}>{isComplete ? t.examCenter.allChecksReady : t.examCenter.reviewInProgress}</h2></div><SetStatusPill status={examSet.status} /></SummaryTop><p style={{ margin: "10px 0 0", color: "rgba(5,5,5,.66)", fontSize: 12, lineHeight: 1.55 }}>The web preview replaces temporary desktop provider files with browser-native narration, illustration, and silent interviewer motion. Publish remains locked until all 16 checks are ready.</p><Meter><MeterFill $value={checks.total ? (checks.ready / checks.total) * 100 : 0} /></Meter><MetricRow><Metric><strong>{checks.narrationReady}/5</strong><span>Narration cues</span></Metric><Metric><strong>{checks.itemReadyCount}/11</strong><span>Prompt items</span></Metric><Metric><strong>{checks.ready}/16</strong><span>Total checks</span></Metric></MetricRow><ItemActions><Button disabled={Boolean(busy)} onClick={() => void act("prepare-media", { examSetId }, "Browser preview media is ready for every item.")}><ArrowPathIcon />{t.examCenter.prepareMedia}</Button>{isComplete && <Button $tone="orange" disabled={Boolean(busy)} onClick={() => void act("set-published", { examSetId, published: examSet.status !== "published" }, examSet.status === "published" ? "The exam is back in media-ready review." : "The exam is published and ready to run.")}><CheckBadgeIcon />{examSet.status === "published" ? t.examCenter.unpublish : t.examCenter.publishExam}</Button>}{examSet.status === "published" && <Button as={Link} href={`/admin/test-center/exams/${examSetId}/preview`} $tone="cream"><RocketLaunchIcon />{t.examCenter.runExam}</Button>}</ItemActions></SummaryCard>
      <InterviewerCard><ExamAvatar interviewer={examSet.interviewer} large /><div><Eyebrow>Selected interviewer</Eyebrow><h2 style={{ margin: 0, fontSize: 20, fontWeight: 900, letterSpacing: "-.04em" }}>{examSet.interviewer.name}</h2><p style={{ margin: "7px 0 12px", color: "rgba(5,5,5,.65)", fontSize: 12, lineHeight: 1.5 }}>{examSet.interviewer.occupation} · {examSet.interviewer.personality} · {examSet.interviewer.voice_tone} voice</p><MediaPill status={examSet.interviewer.video_status} label="Silent listening preview ready" /></div></InterviewerCard>
    </Summary>

    <Section><SectionTop><div><h2>{t.examCenter.sharedNarration}</h2><p>These fixed and authored cues bridge the two task types in a continuous exam run. Use Listen to validate browser voice output.</p></div></SectionTop><NarrationGrid>{examSet.narration.map((cue) => <NarrationCard key={cue.id}><MediaPill status={cue.media_status} /><h3>{cue.label}</h3><p>{cue.script}</p><SmallButton $tone="cream" onClick={() => speak(cue.script)}><SpeakerWaveIcon />{t.examCenter.listen}</SmallButton></NarrationCard>)}</NarrationGrid></Section>
...
function InspectionItem({ item, interviewer, busy, onRetry }: { item: ExamItem; interviewer?: ExamSetDetail["interviewer"]; examSetId: string; busy: boolean; onRetry: () => void }) {
  return <ItemCard key={item.id}>{item.module === "listen_repeat" ? <ItemScene><GardenScene target={item.visual_target} /></ItemScene> : <InterviewerVisual>{interviewer && <ExamAvatar interviewer={interviewer} />}</InterviewerVisual>}<ItemBody><ItemTop><strong>{item.label} · {item.response_seconds}s response</strong><MediaPill status={itemReady(item) ? "ready" : "idle"} /></ItemTop><Prompt>{item.prompt}</Prompt><ItemMeta>{item.module === "listen_repeat" ? `Visual target: ${item.visual_target}. The sentence plays once before recording begins.` : "Question voice is played once; the interviewer remains on screen during the response."}</ItemMeta><ItemActions><SmallButton $tone="cream" onClick={() => speak(item.prompt)}><PlayIcon />Listen</SmallButton><SmallButton disabled={busy} onClick={onRetry}><ArrowPathIcon />Refresh</SmallButton></ItemActions></ItemBody></ItemCard>;
}
