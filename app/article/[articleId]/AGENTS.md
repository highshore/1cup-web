...
- Discussion topics listen to public aggregate scores and reorder by net score. Do not write vote totals from the client; call `voteDiscussionTopic` and treat active subscriptions as the voting entitlement. Legacy topics without IDs use `topic-{originalIndex}`.
