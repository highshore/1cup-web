...
function ActionButton({
  ...
  return (
    <button
      type="button"
      style={{
        ...(secondaryTone ? secondary : primary),
...
function normalizedAudioMime(mimeType: string) {
  const type = mimeType.split(";")[0].toLowerCase();
  return ["audio/webm", "audio/mp4", "audio/mpeg", "audio/ogg", "audio/wav"].includes(type)
    ? type
    : "audio/webm";
}
...
export default function SpeakingTestClient() {
  ...
  const centerCopy = t.speakingCenter;
  ...
  const submitAttempt = useCallback(
    async (responses: CapturedResponse[]) => {
      ...
      try {
        const uploaded = await Promise.all(
          responses.map(async (response) => {
            ...
            const path =
              attempt.uploadPrefix +
              "/" +
              response.itemId +
              "." +
              audioExtension(mimeType);
            const { error } = await supabase.storage
              .from("speaking-test-audio")
        ...
        const payload = await responseJson<{ report: SpeakingTestReport }>(
          await fetch("/api/speaking-test/evaluate", {
            method: "POST",
  ...
  const captureCurrentResponse = useCallback(
    (activeTask: DeployedExamItem) => {
  ...
  const startRecording = useCallback(async () => {
    ...
    try {
      ...
      const recorder = new MediaRecorder(
        stream,
        mimeType ? { mimeType, audioBitsPerSecond: 24_000 } : undefined,
      );
  ...
  const loadHistory = useCallback(async () => {
    try {
      const payload = await responseJson<{ attempts: SpeakingTestAttempt[] }>(
        await fetch("/api/speaking-test/history"),
      );
  ...
  const loadCenterTests = useCallback(async () => {
    setBusy(true);
    setMessage("");
    try {
      ...
      setTests([]);
      setMessage(
        error instanceof Error ? error.message : centerCopy.loadFailed,
      );
    } finally {
      setBusy(false);
    }
  }, [centerCopy.loadFailed, loadHistory]);

  useEffect(() => {
    void loadCenterTests();
  }, [loadCenterTests]);
  ...
  const openTest = useCallback(
    async (examSetId: string) => {
      ...
      try {
        ...
        setMessage(
          error instanceof Error ? error.message : copy.signInRequired,
        );
      } finally {
        setBusy(false);
      }
    ...
    return (
      <main style={page}>
        ...
        >
          ...
          >
            <div>
              ...
              >
                {report.overall.band} · {report.overall.cefr}
              </h1>
              <p
                style={{
                  maxWidth: 590,
            ...
            <strong style={{ fontSize: 23 }}>
              {copy.rawScore} {report.overall.rawScore}/55
            </strong>
          </div>
          <p style={{ margin: "20px 0 0", lineHeight: 1.55 }}>
            <b>{copy.scoreRationale}:</b> {report.overall.rationale}
          </p>
        </div>
        ...
        <section style={{ marginTop: 34 }}>
          ...
          >
            {report.taskScores.map((score) => (
              <div key={score.itemId} style={{ ...card, padding: 17 }}>
                <div
                  style={{
                    display: "flex",
                    justifyContent: "space-between",
                ...
                >
                  <strong>
                    {copy.task} {score.taskNumber} ·{" "}
                    {score.module === "listen_repeat"
                      ? copy.listenRepeat
                      : copy.interview}
                  </strong>
                ...
                >
                  <b>{copy.scoreRationale}:</b> {score.rationale}
                </p>
                <p
                  style={{
                    margin: "10px 0 0",
                ...
                >
                  <b>{copy.evidence}:</b> {score.evidence}
                </p>
                <p
                  style={{
                    margin: "10px 0 0",
                ...
                >
                  <b>{copy.feedback}:</b> {score.feedback}
                </p>
                <div
                  style={{
                    display: "flex",
                ...
                >
                  {Object.entries(score.rubricScores).map(([name, value]) => (
                    <span
                      key={name}
                      style={{
        ...
        >
          <div style={{ ...card, padding: 18 }}>
            <h2 style={{ marginTop: 0, fontSize: 19 }}>{copy.strengths}</h2>
            <ul>
          ...
          <div style={{ ...card, padding: 18 }}>
            <h2 style={{ marginTop: 0, fontSize: 19 }}>{copy.focusAreas}</h2>
            <ul>
        ...
        <ActionButton onClick={returnToCenter}>
  ...
  if (!exam || !task) {
    return (
      <main style={page}>
        <ActionButton onClick={returnToCenter}>{copy.back}</ActionButton>
      </main>
    );
  }
  ...
  return (
    <main style={page}>
      ...
      >
        ...
        >
          {copy.task} {taskIndex + 1} {copy.of} {exam.items.length}
        </span>
        <strong style={{ fontSize: 22 }}>
          {recording ? secondsLeft + " " + copy.seconds : ""}
        </strong>
      </div>
      ...
      <div style={{ ...card, overflow: "hidden" }}>
        ...
        >
          {task.module === "listen_repeat" && task.imageUrl && (
            <Image
              src={task.imageUrl}
              alt=""
              width={900}
          ...
          {task.module === "listen_repeat" && !task.imageUrl && (
            <SpeakerWaveIcon width={58} />
          )}
          {task.module === "interview" && task.videoUrl && (
            <video
              key={task.id}
              autoPlay
              playsInline
        ...
        {task.module === "listen_repeat" && task.audioUrl && (
          <audio
            key={task.id}
            autoPlay
            preload="auto"
        ...
        <div style={{ padding: "20px clamp(18px, 4vw, 32px) 26px" }}>
          ...
          >
            {task.module === "listen_repeat" ? copy.listenRepeat : task.prompt}
          </h1>
          ...
          >
            {!recording ? (
              <ActionButton onClick={() => void startRecording()}>
              ...
              <ActionButton
                secondaryTone
                onClick={() => captureCurrentResponse(task)}
              >
                <PauseCircleIcon width={18} />
                {copy.finishResponse}
              </ActionButton>
            )}
            {recording && (
      ...
      {message && captured.length === exam.items.length && (
        <div style={{ marginTop: 18 }}>
          <ActionButton onClick={() => void submitAttempt(captured)}>
            <ArrowPathIcon width={17} />
            {copy.scoreAgain}
          </ActionButton>
        </div>
      )}
