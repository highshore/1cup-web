...
// Colors map to the theme tokens in app/globals.css, which mirror
// app/lib/constants/colors.ts (text-ink, text-ink-medium, border-line, …).
const contentClass =
  "text-base leading-[1.8] text-ink-medium max-[768px]:text-[0.95rem] " +
  "[&_ul]:mt-2 [&_ul]:mr-0 [&_ul]:mb-4 [&_ul]:ml-[1.35rem] [&_ul]:list-disc [&_ul]:p-0 " +
  "[&_ol]:mt-2 [&_ol]:mr-0 [&_ol]:mb-4 [&_ol]:ml-[1.35rem] [&_ol]:list-decimal [&_ol]:p-0 " +
  "[&_li]:list-item [&_li]:mb-[0.4rem] [&_strong]:text-ink";

const sectionClass = "mb-9";

const sectionTitleClass =
  "mb-4 text-[1.35rem] text-primary max-[768px]:text-[1.18rem]";

const paragraphClass = "mt-0 mb-4";

const noteClass =
  "my-4 rounded-[10px] border border-line bg-gray-light px-[1.1rem] py-4";

const dividerClass = "my-8 border-0 border-t border-line";
  ...
  return (
    <div className={contentClass}>
      ...
      <section className={sectionClass}>
        ...
        <p className={paragraphClass}>회사는 다음 목적을 위하여 필요한 범위에서 개인정보를 처리합니다.</p>
        <ul>
          ...
          <li><strong>멤버십·회차 참여권 및 결제:</strong> 지역별 상품과 요금 적용, 멤버십 가입·갱신·해지, 기존 구독 가격 유지 여부 관리, 회차 참여권 구매·사용·반환·만료·환불, 추천인 할인 운영, 거래내역 및 환불 가능 금액 확인</li>
          <li><strong>밋업 운영:</strong> 이용권의 사용 가능 지역 확인, 밋업 신청, 참가자 및 정원 관리, 일정 안내, 운영상 필요한 연락</li>
      ...
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>2. 처리하는 개인정보의 항목 및 수집 방법</h2>
        <p className={paragraphClass}><strong>회원가입·인증</strong></p>
        <ul>
        ...
        <p className={paragraphClass}><strong>프로필·커뮤니티</strong></p>
        <ul>
          <li>표시 이름, 프로필 이미지, 자기소개, 직장·업무 정보, 학교, 활동지역, 관심사, 프로필 공개 여부</li>
          <li>좋아요·차단 정보, 대화 및 메시지 내용, 피드백·설문 응답</li>
        </ul>
        <p className={paragraphClass}><strong>멤버십·회차 참여권·결제</strong></p>
        <ul>
          <li>회원 식별자, 주문번호, 상품 종류, 이용 지역, 가격정책 버전, 표시가격·할인금액·실제 결제금액, 결제상태·수단·일시, 정기결제용 빌링키, 멤버십 시작·종료일 및 현재 갱신금액</li>
          <li>참여권 구매 수량, 잔액, 유효기간, 구매·사용·반환·만료·환불 거래 원장, 관련 밋업 및 결제 주문 연결정보</li>
          <li>추천인 코드, 추천인 회원 식별자, 적용 상품·지역·할인금액, 사용·해제 상태 및 시각</li>
          <li>결제·자동결제 중단·승인취소·부분 또는 전액 환불 내역과 환불 사유 등 거래 처리에 필요한 정보</li>
          <li>회사는 카드번호 전체 또는 카드 보안코드(CVC)를 직접 저장하지 않으며 카드 결제정보는 결제대행사를 통해 처리합니다.</li>
        </ul>
        <p className={paragraphClass}><strong>밋업·영어 학습·말하기 분석</strong></p>
        <ul>
          <li>밋업 신청·참가 내역, 이용권 종류, 참가자 역할 등 운영에 필요한 정보</li>
          <li>저장 단어, 학습·복습 기록, 말하기 테스트 응답, 발화 전사문, 말하기 시간·발화량·속도·어휘 다양성 등 분석 지표, AI 평가·피드백</li>
        </ul>
        <p className={paragraphClass}><strong>기타 신청 및 자동 생성 정보</strong></p>
        <ul>
      ...
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>3. 개인정보의 처리 및 보유기간</h2>
        <p className={paragraphClass}>
        ...
        <ul>
          ...
          <li>대금결제, 멤버십·참여권 공급 및 참여권 거래 원장에 관한 기록: 5년</li>
      ...
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>5. 개인정보 처리업무의 위탁</h2>
        <p className={paragraphClass}>회사는 서비스 제공을 위해 다음 업체에 개인정보 처리업무의 일부를 위탁합니다.</p>
        <ul>
          <li><strong>Supabase, Inc.</strong> — 회원 인증, 데이터베이스, 파일 저장, 멤버십·참여권·밋업·결제 거래 데이터 처리 및 백엔드 인프라</li>
      ...
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>6. 개인정보의 국외 이전</h2>
        <p className={paragraphClass}><strong>Supabase, Inc.</strong></p>
        <ul>
          <li>이전 국가: 싱가포르</li>
          <li>이전 항목: Supabase 인프라에 저장·처리되는 회원, 인증, 프로필, 멤버십, 회차 참여권, 이용 지역, 결제·추천인·환불, 밋업, 커뮤니티, 학습 및 서비스 이용정보</li>
        ...
        <p className={paragraphClass}><strong>OpenAI OpCo, LLC</strong></p>
        <ul>
      ...
      <section className={sectionClass}>
        ...
        <ul>
          ...
          <li>법령에 따라 보존해야 하는 결제기록과 멤버십·참여권 거래 원장 내역은 해당 보존기간 동안 별도로 보관합니다. 회원탈퇴 시 직접 식별정보 및 자유기재 사유는 삭제·익명화하고, 금액·주문 연결·발생 시각 등 필요한 감사기록만 유지합니다.</li>
          <li>통계·연구 등으로 계속 활용할 정보는 제8조의 가명처리 절차와 보호조치를 적용합니다.</li>
        </ul>
      </section>

      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>8. 가명정보의 처리</h2>
        <p className={paragraphClass}>
      ...
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>9. 이용자의 권리와 행사 방법</h2>
        <p className={paragraphClass}>
      ...
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>10. 개인정보의 안전성 확보조치</h2>
        <ul>
      ...
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>11. 개인정보 보호책임자 및 문의처</h2>
        <ul>
      ...
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>12. 개인정보처리방침의 변경</h2>
        <p className={paragraphClass}>
          회사는 법령, 서비스 또는 개인정보 처리방식의 변경에 따라 본 방침을 수정할
          수 있습니다. 중요한 변경은 서비스 내 공지 등 합리적인 방법으로 변경 내용과
          시행일을 안내합니다.
        </p>
        <p className={paragraphClass}>공고일: {EFFECTIVE_DATE}<br />시행일: {EFFECTIVE_DATE}</p>
      </section>
    </div>
  );
}
...
function TermsOfService() {
  return (
    <div className={contentClass}>
      ...
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>제2조 (정의)</h2>
        <ol>
          ...
          <li>“멤버십”이란 회원이 이용요금을 결제하고 일정 기간 동안 결제 시 선택하거나 적용된 지역의 유료 서비스와 밋업 등에 접근할 수 있는 이용권을 말합니다.</li>
          <li>“멤버십 기간”은 별도 안내가 없는 한 각 결제 완료 시점부터 정확히 30일(720시간)입니다. 회사가 프로모션, 장애 보상 또는 밋업 취소 보상 등으로 기간을 추가한 경우 그 추가 기간도 포함됩니다.</li>
          <li>“회차 참여권”이란 구매 시 표시된 지역의 밋업 1회 신청에 사용할 수 있는 선불형 이용권입니다. 회차 참여권은 멤버십 또는 자동결제가 아니며, 별도 상품으로 구매·관리됩니다.</li>
          <li>“이용 지역”이란 멤버십 또는 회차 참여권으로 신청할 수 있는 밋업의 지역 범위를 말하며 결제 화면에 표시됩니다.</li>
      ...
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>제3조 (약관의 게시 및 변경)</h2>
        <ol>
      ...
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>제4조 (회원가입 및 계정)</h2>
        <ol>
      ...
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>제5조 (서비스, 멤버십 및 이용 지역)</h2>
        <ol>
          <li>회사는 결제 당시 안내된 범위의 유료 서비스를 멤버십 회원에게 제공합니다.</li>
          <li>신규 지역형 멤버십은 결제 시 선택한 이용 지역의 밋업에 적용됩니다. 별도 표시가 없는 한 다른 지역의 밋업 신청 권한까지 자동으로 포함하지 않습니다.</li>
          <li>회차 참여권 역시 구매 시 선택한 이용 지역의 밋업에 사용됩니다. 다만 지역 정보가 도입되기 전에 발급된 일부 기존 참여권은 발급 당시 조건에 따라 지역 제한 없이 사용할 수 있습니다.</li>
          <li>정원이 있는 밋업은 신청·예약 완료 순서 등 사전에 안내한 기준에 따라 참가가 제한될 수 있으며, 멤버십이나 참여권 보유만으로 모든 밋업 좌석이 보장되지는 않습니다.</li>
          <li>밋업의 일정, 장소, 진행자, 주제 및 정원은 운영상 필요에 따라 변경될 수 있으며 중요한 변경은 가능한 범위에서 미리 안내합니다.</li>
          <li>회사의 사정으로 예정된 밋업이 취소되는 경우 원칙적으로 멤버십 회원의 멤버십 기간을 2주 연장합니다. 회차 참여권으로 신청한 회원에게는 사용한 참여권 1회를 반환합니다. 회사가 회원에게 더 유리한 대체 보상 또는 환불을 제공하는 경우에는 그에 따를 수 있습니다.</li>
          <li>천재지변, 정부 명령, 시설의 갑작스러운 이용 불가 등 회사가 합리적으로 통제하기 어려운 사유가 있는 경우 일정 변경, 기간 연장 또는 이에 상응하는 조치를 할 수 있습니다.</li>
        </ol>
      </section>

      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>제6조 (밋업 및 커뮤니티 이용수칙)</h2>
        <p className={paragraphClass}>회원은 다른 참가자의 안전과 권리를 존중해야 하며 다음 행위를 해서는 안 됩니다.</p>
        <ul>
        ...
        <p className={paragraphClass}>
          ...
          다만 멤버십 전체의 해지·환불 권리는 제9조, 회차 참여권의 환불은 제9조의2에 따릅니다.
        </p>
      </section>

      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>제7조 (이용요금, 지역별 가격 및 결제)</h2>
        <ol>
          <li>멤버십 및 회차 참여권의 이용요금, 이용 지역, 할인, 수량, 유효기간 및 결제주기는 결제 화면에 표시된 내용을 따릅니다. 상품 가격은 이용 지역별로 다를 수 있습니다.</li>
          <li>회차 참여권은 결제 화면에 표시된 수량으로 한 번만 결제되며 자동결제 또는 멤버십 상태 변경을 발생시키지 않습니다.</li>
          <li>기존에 연속해서 유지 중인 정기구독 회원에게 회사가 별도 가격 보호를 적용한 경우 해당 구독이 중단되지 않는 동안 기존 실제 결제금액이 유지될 수 있습니다. 기존 구독을 종료한 뒤 다시 가입하면 재가입 시점의 상품 가격과 조건이 적용됩니다.</li>
          <li>추천인 코드는 원칙적으로 회원의 첫 번째 성공한 유료 구매에 한 번만 사용할 수 있으며 본인의 추천 코드는 사용할 수 없습니다. 실제 할인금액과 적용 조건은 결제 화면에 표시된 내용을 따릅니다.</li>
          <li>추천 할인이 적용된 정기 멤버십은 별도 안내가 없는 한 구독이 중단되지 않는 동안 해당 실제 결제금액으로 갱신될 수 있습니다.</li>
      ...
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>제8조 (자동결제 및 다음 결제 중단)</h2>
        <ol>
      ...
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>제9조 (멤버십 해지, 청약철회 및 환불)</h2>
        <ol>
      ...
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>제9조의2 (회차 참여권의 사용, 취소 및 환불)</h2>
        <ol>
          <li>회차 참여권 1회는 해당 참여권의 이용 지역에 속하는 밋업 1건의 참가 신청에 사용됩니다. 같은 지역의 유효한 멤버십이 있거나 운영상 무상 참가 대상인 경우에는 참여권이 차감되지 않습니다.</li>
          <li>현재 판매하는 회차 참여권은 구매 완료 시점부터 180일 동안 유효하며, 유효기간이 지나면 사용되지 않은 잔액은 소멸합니다. 상품별 수량과 유효기간은 결제 화면에 표시된 내용을 따릅니다.</li>
          <li>회차 참여권으로 신청한 밋업은 시작 24시간 전까지 취소하면 해당 신청에 사용한 참여권 1회가 자동 반환됩니다. 그 이후에는 반환되지 않으며, 회사가 밋업을 취소하는 경우에는 신청에 사용한 참여권 1회를 반환합니다.</li>
          <li><strong>남은 참여권 환불:</strong> 유효기간 내에는 구매 건별 실제 결제금액에 남은 횟수 비율을 적용해 환불합니다. 환불액은 <strong>실제 결제금액 × 남은 횟수 ÷ 구매한 총 횟수</strong>로 계산하고 원 단위에서 반올림합니다. 현재 5회 참여권의 경우 실제 결제금액 × 남은 횟수 ÷ 5로 계산합니다.</li>
          <li>추천 할인을 받은 참여권은 할인 후 실제 결제금액을 기준으로 환불액을 계산합니다. 환불이 완료되면 해당 구매 건의 남은 참여권은 모두 회수됩니다.</li>
          <li>유효기간이 지난 참여권 또는 남은 횟수가 없는 구매 건은 환불되지 않습니다. 다만 회사 귀책 또는 관계 법령에 따른 환불·구제는 관계 법령과 개별 안내에 따릅니다.</li>
          <li>참여권의 구매·사용·반환·환불은 거래 원장에 기록되며, 같은 결제 또는 같은 참가 신청에 대하여 중복으로 처리되지 않습니다.</li>
        </ol>
      </section>

      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>제10조 (회원탈퇴)</h2>
        <ol>
          ...
          <li>회원탈퇴와 자동결제 중단, 멤버십·회차 참여권의 해지·환불은 서로 다른 절차이며, 회원탈퇴 자체가 관계 법령상 인정되는 정당한 환불권을 소멸시키지는 않습니다.</li>
          <li>회원탈퇴 시 회사는 법령상 보존의무가 있는 거래기록 등을 제외한 계정 및 개인정보를 삭제·익명화하고, 통계·과학적 연구 목적으로 적법하게 가명처리한 정보는 개인정보처리방침에 따라 별도로 처리할 수 있습니다.</li>
        </ol>
      </section>

      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>제11조 (서비스 이용 제한 및 계약 해지)</h2>
        <p className={paragraphClass}>
          ...
          지체 없이 사유를 안내합니다. 유료 계약 종료에 따른 환불은 제9조와 제9조의2,
          관계 법령에 따릅니다.
        </p>
      </section>

      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>제12조 (콘텐츠 및 지식재산권)</h2>
        <ol>
      ...
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>제13조 (녹음, 전사 및 AI 분석)</h2>
        <ol>
      ...
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>제14조 (서비스의 변경 및 중단)</h2>
        <ol>
      ...
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>제15조 (개인정보 보호)</h2>
        <p className={paragraphClass}>회사는 회원의 개인정보를 「개인정보 보호법」 등 관계 법령과 회사의 개인정보처리방침에 따라 처리합니다.</p>
      </section>

      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>제16조 (회원에 대한 통지)</h2>
        <p className={paragraphClass}>
      ...
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>제17조 (책임의 범위)</h2>
        <p className={paragraphClass}>
      ...
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>제18조 (준거법 및 분쟁해결)</h2>
        <ol>
      ...
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>사업자 정보</h2>
        <ul>
          <li>상호: 네이티브피티</li>
...
function RefundPolicy() {
  return (
    <div className={contentClass}>
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>멤버십 기간 및 이용 지역</h2>
        <p className={paragraphClass}>
          성공적인 각 결제는 결제 완료 시점부터 정확히 30일(720시간)의 멤버십을
          제공합니다. 신규 지역형 멤버십은 결제 시 선택한 지역의 밋업에 적용되며,
          프로모션 또는 보상으로 별도 기간이 추가된 경우에는 그 기간도 함께 적용됩니다.
        </p>
      </section>
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>멤버십 전액 환불</h2>
        <p className={paragraphClass}>
      ...
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>멤버십 7일 이후 부분 환불</h2>
        <p className={paragraphClass}>
      ...
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>자동결제 중단과 즉시 해지의 차이</h2>
        <ul>
          ...
          <li><strong>기존 회원 가격 보호:</strong> 별도 가격 보호가 적용된 기존 활성 구독은 구독이 중단되지 않는 동안 기존 실제 결제금액이 유지될 수 있으며, 종료 후 재가입 시에는 당시의 신규 가격과 조건이 적용됩니다.</li>
        </ul>
      </section>
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>밋업 관련</h2>
        <p className={paragraphClass}>
          회원 개인 사정으로 특정 밋업에 참석하지 못한 경우 그 불참만을 이유로
          별도 환불 또는 기간 연장을 제공하지 않습니다. 회사 사정으로 밋업이
          취소되는 경우에는 원칙적으로 멤버십 회원의 멤버십 기간을 2주 연장하고,
          회차 참여권으로 신청한 회원에게는 사용한 참여권 1회를 반환합니다.
          관계 법령이 회원에게 더 유리한 권리를 인정하는 경우에는 해당 법령이 우선합니다.
        </p>
      </section>
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>회차 참여권</h2>
        <p className={paragraphClass}>
          회차 참여권은 멤버십과 별개의 1회성 선불 이용권입니다. 현재 판매하는 5회
          참여권은 한 번만 결제되고 자동결제는 발생하지 않으며, 구매 완료 시점부터
          <strong>180일</strong> 동안 구매 시 선택한 지역의 밋업에 사용할 수 있습니다.
        </p>
        <ul>
          <li><strong>사용:</strong> 같은 지역의 유효한 멤버십 또는 무상 참가 대상이 아닌 회원이 밋업을 신청하면 1회가 차감됩니다.</li>
          <li><strong>밋업 취소:</strong> 밋업 시작 24시간 전까지 직접 취소하면 차감한 1회가 자동 반환됩니다. 이후 취소에는 반환되지 않습니다.</li>
          <li><strong>회사 취소:</strong> 회사가 밋업을 취소하면 해당 신청에 사용한 1회가 반환됩니다.</li>
          <li><strong>남은 참여권 환불:</strong> 유효기간 내에는 실제 결제금액 × 남은 횟수 ÷ 구매한 총 횟수로 계산한 금액을 환불합니다. 현재 5회 참여권은 실제 결제금액 × 남은 횟수 ÷ 5로 계산합니다.</li>
          <li><strong>추천 할인 적용 건:</strong> 할인 후 실제 결제금액을 기준으로 환불액을 계산합니다.</li>
          <li><strong>환불 후:</strong> 환불이 완료되면 해당 구매 건의 남은 참여권은 모두 회수됩니다.</li>
          <li><strong>만료 후:</strong> 유효기간이 지난 참여권 또는 남은 횟수가 없는 구매 건은 환불되지 않습니다. 법령상 더 유리한 권리가 있거나 회사 귀책이 있는 경우에는 그 기준을 우선합니다.</li>
          <li><strong>기존 참여권:</strong> 지역 정보가 도입되기 전에 발급된 일부 기존 참여권은 발급 당시 조건에 따라 지역 제한 없이 사용할 수 있습니다.</li>
        </ul>
      </section>
      <section className={sectionClass}>
        <h2 className={sectionTitleClass}>신청 방법 및 환급</h2>
        <p className={paragraphClass}>
          멤버십 즉시 해지·환불과 다음 결제 중단은 서비스 내 계정 또는 프로필에서
          제공하는 기능을 통해 신청할 수 있습니다. 회차 참여권 환불은 참여권 환불
          페이지 또는 고객지원을 통해 구매 건별 환불 가능 여부와 예상 금액을 확인한 뒤
          신청할 수 있습니다. 환불은 원칙적으로 기존 결제수단을 통해 처리되며 실제 카드
          승인취소·환급 반영 시점은 결제사업자 또는 금융기관의 처리기간에 따라 달라질 수 있습니다.
        </p>
        <p className={paragraphClass}>시행일: {EFFECTIVE_DATE}</p>
      </section>
    </div>
  );
}
...
export function PolicyClient() {
  ...
  const policy =
    type === "terms"
      ...
      : type === "refund"
        ? { title: "환불 및 이용권 해지 정책", content: <RefundPolicy /> }
        : { title: "개인정보처리방침", content: <PrivacyPolicy /> };

  return (
    <div className="mx-auto mt-[60px] mb-0 min-h-screen max-w-[880px] px-6 pt-8 pb-20 font-[system-ui,-apple-system,BlinkMacSystemFont,'Segoe_UI',Roboto,'Helvetica_Neue',Arial,sans-serif] max-[768px]:px-4 max-[768px]:pt-6 max-[768px]:pb-16">
      <h1 className="mb-3 text-center text-[2rem] text-ink max-[768px]:text-[1.8rem]">
        {policy.title}
      </h1>
      <p className="mt-0 mb-10 text-center text-[0.92rem] text-ink-medium">
        시행일 {EFFECTIVE_DATE}
      </p>
      {policy.content}
    </div>
